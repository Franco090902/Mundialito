...
function parsearCalendario(textoRespuesta) {
  ...
  let   jsonParte = textoRespuesta.substring(idx + separador.length).trim();

  // Gemini a veces agrega texto después del JSON (ej: explicaciones).
  ...
  try {
    ...
    // Normalizar: si Gemini devolvió el Caso A, aseguramos que los
    // campos hora_arg y fecha tengan valores de fallback razonables.
    if (calendario.equipo1 && calendario.equipo2) {
      if (!calendario.hora_arg || !/^\d{2}:\d{2}$/.test(calendario.hora_arg)) {
        calendario.hora_arg = '18:00'; // hora por defecto si Gemini no la conoce
      }
      if (!calendario.fecha || !/^\d{4}-\d{2}-\d{2}$/.test(calendario.fecha)) {
        calendario.fecha = '2026-06-15'; // fecha por defecto si falta
      }
    }
